...
Both approaches use cosine similarity and the same ranking logic. The queried movie is excluded from its own recommendations, and equal similarity scores are resolved using an alphabetical tie-break on normalized titles.

The comparison was run against the project's real 1,000-movie dataset using three sample queries: **The Dark Knight**, **Interstellar**, and **Doctor Strange**.
...
**The Dark Knight Rises** is a particularly intuitive result because it shares substantial narrative and thematic vocabulary with the queried movie. The other recommendations demonstrate that description similarity can identify textual relationships that are not visible from genre labels alone.

### Interpretation

For this query, the description-based approach provides more informative ranking because the genre representation produces several exact ties. The genre approach still provides useful broad-category similarity, but it lacks enough detail to distinguish movies sharing the same genres.
...
The Martian receives a perfect genre similarity score, indicating that its genre representation matches Interstellar's representation exactly.
...
For this query, the genre approach gives broad-category matches, while the description approach exposes the limitations of lexical similarity. The description results are not necessarily bad—the algorithm is correctly finding shared textual signals—but those signals do not always correspond to what a human would consider a useful recommendation.
...
The project therefore uses normalized titles as the primary alphabetical tie-break. This makes the recommendation output reproducible and predictable.
